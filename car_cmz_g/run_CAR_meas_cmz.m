...
%DRA.prepareUWVMeas(moreInputData);
DRA.prepareUWVMeasCMZ(moreInputData);
...
SAVEMOVIE = true;
if SAVEMOVIE
    vidObj = VideoWriter('video/car_maeas_cmz.avi');
    vidObj.FrameRate=20;
    open(vidObj);
end
...
%             [xy, latency] = nm.dataparser.getMocapPos(rb, walltime);
    ...
    if walltime >= plotZonotime + DRA.timeStepDataIn*stepsReach
           R_data=DRA.reachDataMeasCMZ(zonotope([[z(1);z(2)]]),stepsReach);
           for ii =2:stepsReach+1
               %updatePlotZono(h_reach(ii),zonotope(R_data{ii}),[1,2],'r');
               plotCZono(R_data{ii});
               R_history{R_history_index}=zonotope(R_data{ii});
