...
if parking
%load('cache\theta30Zono.mat')
load('cache\parkingconst_Zono.mat')
plotStep =15;
else
load('cache\roundZono.mat')
plotStep =10;
end
...
for i=1:plotStep:length(R_history_Inter)
   reachHandle=plot( R_history_Inter{i},[1 2],'r');
end
% label plot
xlabel(['x_{',num2str(1),'}']);
ylabel(['x_{',num2str(2),'}']);
...
warOrig = warning; warning('off','all');
legend([trajHandle,reachHandle],...
    'Trajectory', 'Set from data $\bar{\mathcal{Z}}_k$','Location','northwest','Interpreter','latex');
warning(warOrig);
ax = gca;
ax.FontSize = 23;
%set(gcf, 'Position',  [50, 50, 800, 400])
